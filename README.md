...
파일을 업로드하고 공유 가능한 다운로드 링크를 받습니다.
...
```json
{
  "url": "https://golang-fileshared.onrender.com/d/{id}/{filename}?token={token}"
}
```

**상태 코드:**
- `200` - 성공
- `400` - 잘못된 요청 (파일 없음, 형식 불일치, 크기 초과)
- `500` - 서버 오류
...
[
  {
    ...
    "uploaded_at": "string",  // 업로드 시간 (ISO8601)
    "url": "string",          // 다운로드 URL (토큰 포함)
...
### GET /d/{id}/{filename}?token={token}

파일을 다운로드합니다. HMAC 토큰 검증이 필수입니다.

**요청:**
- Method: `GET`
- Query Params: `token` (필수)

**응답:**
- 이미지 파일 (`.png`, `.jpg`, `.jpeg`): 브라우저에서 미리보기 (`Content-Disposition: inline`)
- 기타 파일: 다운로드 (`Content-Disposition: attachment`)
